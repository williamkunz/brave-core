...
LedgerDatabase::LedgerDatabase(const base::FilePath& db_path) :
    db_path_(db_path),
    initialized_(false) {
...
DBCommandResponsePtr LedgerDatabase::RunTransaction(
    ledger::DBTransactionPtr transaction) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

  auto command_response = ledger::DBCommandResponse::New();
...
ledger::DBCommandResponse::Status LedgerDatabase::Initialize(
  ...
  if (!initialized_) {
    ...
    memory_pressure_listener_.reset(new base::MemoryPressureListener(
        FROM_HERE,
        base::Bind(&LedgerDatabase::OnMemoryPressure,
...
ledger::DBCommandResponse::Status LedgerDatabase::Execute(
...
ledger::DBCommandResponse::Status LedgerDatabase::Run(
    ledger::DBCommand* command) {
  if (!initialized_) {
    return ledger::DBCommandResponse::Status::INITIALIZATION_ERROR;
  }
...
ledger::DBCommandResponse::Status LedgerDatabase::Read(
    ledger::DBCommand* command,
    ledger::DBCommandResponse* command_response) {
  if (!initialized_) {
    return ledger::DBCommandResponse::Status::INITIALIZATION_ERROR;
...
ledger::DBCommandResponse::Status LedgerDatabase::Migrate(
    const int32_t version,
    const int32_t compatible_version) {
  if (!initialized_) {
    return ledger::DBCommandResponse::Status::INITIALIZATION_ERROR;
