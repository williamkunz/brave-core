...
IN_PROC_BROWSER_TEST_F(HTTPSEverywhereServiceTest, RedirectsKnownSite) {
  ...
  EXPECT_EQ(GURL("https://www.01.org/"), contents->GetLastCommittedURL().ReplaceComponents(clear_port));
}
...
IN_PROC_BROWSER_TEST_F(HTTPSEverywhereServiceTest, RedirectsKnownSiteInIframe) {
  ...
  EXPECT_EQ(GURL("https://www.01.org/"),
            iframe_contents->GetLastCommittedURL().ReplaceComponents(clear_port));
}
