 ...
 public:
  ...
  void SetUpOnMainThread() override {
    ...
    https_server_.RegisterRequestHandler(base::BindRepeating(
        &BraveThumbnailDatabaseBrowserTest::HandleRequest,
        base::Unretained(this)));
    https_server_.RegisterRequestMonitor(base::BindRepeating(
        &BraveThumbnailDatabaseBrowserTest::SaveRequest,
        base::Unretained(this)));
  ...
  std::unique_ptr<net::test_server::HttpResponse> HandleRequest(
    ...
    if (!base::ReadFileToString(test_data_dir_.Append("favicon.ico"),
                                &file_contents)) {
      return nullptr;
    }
...
IN_PROC_BROWSER_TEST_F(BraveThumbnailDatabaseBrowserTest,
                       SetRead1001) {
  ClearRequests();
