...
class BraveOmniboxViewViewsTest : public InProcessBrowserTest,
 ...
 public:
  ...
  void CheckTorInitProgress(OmniboxViewViews* tor_views,
                            const std::string& percentage) {
    static_cast<BraveOmniboxViewViews*>(tor_views)
      ->OnTorInitializing(percentage);
...
IN_PROC_BROWSER_TEST_F(BraveOmniboxViewViewsTest, TorInitProgress) {
  ...
  // Emulate tor init progress
  CheckTorInitProgress(tor_views, "50");
  CheckTorInitProgress(tor_views, "80");
  CheckTorInitProgress(tor_views, "90");

  // Emulate tor process connected
  static_cast<BraveOmniboxViewViews*>(tor_views)->OnTorCircuitEstablished(true);
  EXPECT_FALSE(
      base::StartsWith(tor_views->GetPlaceholderText(),
                       l10n_util::GetStringUTF16(IDS_OMNIBOX_INITIALIZING_TOR),
                       base::CompareCase::SENSITIVE));
  EXPECT_FALSE(
      base::StartsWith(tor_views->GetText(),
                       l10n_util::GetStringUTF16(IDS_OMNIBOX_INITIALIZING_TOR),
                       base::CompareCase::SENSITIVE));
